...
const PREFLIGHT_ENABLED =
  process.env.MARKET_CREATOR_PREFLIGHT === "1" || Boolean(process.env.MIMIR_APP_URL?.trim());
const PREFLIGHT_APP_URL = process.env.MIMIR_APP_URL ?? "http://localhost:3000";
...
async function draftClaimCandidates(sourceData: {
  ...
  const prompt = `You are Mimir, an AI that creates high-quality prediction market claims for a USDC market on Somnia.
...
async function run(): Promise<void> {
  ...
  console.log(`[market-creator] Balance : ${weiToStt(balance).toFixed(4)} ETH`);
...
async function main(): Promise<void> {
  ...
  console.log(`  Preflight  : ${PREFLIGHT_ENABLED ? `on via ${PREFLIGHT_APP_URL}` : "off"}`);
